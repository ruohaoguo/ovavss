...
from .matcher import VideoHungarianMatcher
...
class VideoMaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            return retry_if_cuda_oom(self.inference_video)(self.num_queries, self.sem_seg_head.num_classes, mask_cls_result, mask_pred_result, image_size, height, width)

    def postprocess(self, mask_cls_result, mask_pred_result, image_hw):
    ...
    @classmethod
    def prepare_targets(cls, num_frames, targets, images):
    ...
    def prepare_targets_avsbench(self, targets, images):
        h_pad, w_pad = images.tensor.shape[-2:]
        gt_instances = []
        for targets_per_video in targets:
            if targets_per_video["label"] == "v1s":
                _num_instance = len(targets_per_video["instances"][0])
                ...
                targets_per_frame.gt_classes = targets_per_frame.gt_classes.to(self.device)
                gt_classes_per_video = targets_per_frame.gt_classes[valid_idx]  # N,
                gt_ids_per_video = gt_ids_per_video[valid_idx]  # N, num_frames

                gt_instances.append({"labels": gt_classes_per_video.to(self.device), "ids": gt_ids_per_video})
    ...
    @classmethod
    def inference_video(cls, num_queries, num_classes, pred_cls, pred_masks, img_size, output_height, output_width):
        if len(pred_cls) > 0:
