...
class VideoSetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets, video_labels):
        ...
        """
        for video_i in range(len(video_labels)):
            if video_labels[video_i] == "v1s":
                outputs["pred_masks"] = outputs["pred_masks"][video_i, :, 0:1, :, :].unsqueeze(0)
                aux_outputs_ = []
                for aux_i in outputs["aux_outputs"]:
                    aux_i["pred_masks"] = aux_i["pred_masks"][video_i, :, 0:1, :, :].unsqueeze(0)
                    aux_outputs_.append(aux_i)
                outputs["aux_outputs"] = aux_outputs_
...
class VideoSetTrackingCriterion(VideoSetCriterion):
    """This class computes the loss for DETR.
    The process happens in two steps:
        1) we compute hungarian assignment between ground truth boxes and the outputs of the model
        2) we supervise each pair of matched ground-truth / prediction (supervise class and box)
